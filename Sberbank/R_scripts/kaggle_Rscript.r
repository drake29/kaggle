...
sq_graph = data.train %>% 
  filter(full_sq < 2000) %>% 
  ggplot(data.train, aes(x=full_sq, y=price_doc)) + geom_point()

  
data.train %>% 
  filter(full_sq < 2000) %>% 
  ggplot(aes(x = full_sq, y=price_doc))  + ggtitle('Price by area in sq_meters') +
  geom_point(color='red', alpha=0.5) + geom_smooth(method='lm')
  labs(x='Area', y='price')
  
hist(data.train$num_room)
unique(data.train$num_room)
table(data.train$num_room)

data.train %>% 
  ggplot(aes(x= num_room, y=price_doc)) + 
  geom_col(color='blue', alpha=0.5) + geom_smooth(method='lm')
...
data %>% 
  ggplot(aes(x= year, y= price_doc)) + geom_point() + geom_smooth()
